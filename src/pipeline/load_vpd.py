...
def find_vpd_files(raw_dir: Path | None = None) -> list[Path]:
    """VPD workbooks are identified by filename, not a fixed name -- any .xlsx
    whose name contains 'VPD' (case-insensitive). `raw_dir` defaults to this
    project's data/raw/; the web app passes a per-job temp directory."""
    raw_dir = raw_dir or (PROJECT_ROOT / "data" / "raw")
    return sorted(p for p in raw_dir.glob("*.xlsx") if "vpd" in p.name.lower())
